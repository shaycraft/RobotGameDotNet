...
namespace robotgamedotnet.Robots
{
    public class ExampleRobot : Robot
    {
        public override Action act()
        {
            ...
            {
                if (bot.player.id != this.player.id)
                {
                    if (rg.distance(bot.location, this.location) <=1)
                    {
                        return new Action { Type = ActionType.Attack, AttackLocation = bot.location };
                    }
                }
            }

            // move towards the center
            return new Action { Type = ActionType.Move, MoveLocation = rg.toward(this.location, rg.CENTER_POINT(this.board)) };
        }
    }
}
